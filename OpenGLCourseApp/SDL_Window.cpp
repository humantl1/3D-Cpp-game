...
int SDL_Window::Initialize() {
  if (SDL_Init(SDL_INIT_EVERYTHING) !=
      0) {  // Initialize all SDL subsystems & check for success
  ...
  SDL_GL_SetAttribute(  
      // disable funtions depricated in specified version
      SDL_GL_CONTEXT_PROFILE_MASK,
      SDL_GL_CONTEXT_PROFILE_CORE);

  SDL_GL_SetAttribute(
      SDL_GL_CONTEXT_FLAGS,
      // in windows, disable future deprecated functionality
  ...
  window = SDL_CreateWindow(   // create window via SDL API
      ...
      SDL_WINDOW_OPENGL |
          SDL_WINDOW_BORDERLESS);  // flags (opengl usage, no border)
  ...
  gl_context = SDL_GL_CreateContext(
      window);  // create OpenGL context associated with window
  if (!gl_context) {
    fprintf(stderr,
            "ERROR: OpenGL context could not be created! SDL Error: %s\n",
            SDL_GetError());
    return 1;
  }
